...
//James Le Modified to work with previous hashmap
 ...
 */
uint16_t hashLSP(uint16_t k){
	return k%13;
}
uint16_t hash2LSP(uint16_t k){
	return 1+k%11;
}
 ...
 */
uint16_t hash3LSP(uint16_t k, uint16_t i){
 ...
 */
void hashmapInitLSP(hashmapLSP *input){
	uint16_t i;
...
bool hashmapIsEmptyLSP(hashmapLSP *input){
 ...
 */
hashTypeLSP hashmapGetLSP(hashmapLSP *input, uint8_t key){
	uint16_t i=0;	uint16_t j=0;
	do{
		j=hash3LSP(key, i);
		if(input->map[j].key == key){	return input->map[j].value;}
		i++;
	}while(i<LSPHASH_MAX_SIZE);	
 ...
 * 			uint8_t key = location key
 */
bool hashmapContainsLSP(hashmapLSP *input, uint8_t key){
	uint16_t i=0;	uint16_t j=0;
 ...
 */
void hashmapInsertLSP(hashmapLSP *input, uint8_t key, hashTypeLSP value){
	uint16_t i=0;	uint16_t j=0;
 ...
 * 			uint8_t key = location key
 */
void hashmapRemoveLSP(hashmapLSP *input, uint8_t key){
	uint16_t i=0;	uint16_t j=0;
	do{
		j=hash3LSP(key, i);
